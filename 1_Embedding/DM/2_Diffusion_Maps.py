...
# ====================================================================================
# Diffusion Maps (run via 'python 2_Diffusion_Maps.py') 
...
tau = 1 #set to match previous value used (number of noisy-duplicates)

if 0: #Times font for all figures
    rc('text', usetex=True)
    rc('font', family='serif')

PD = '001' #index starts at zero
Dist = np.load(os.path.join(dataDir, 'PD%s_tau1_dist.npy' % PD)) #distances from projections of MRC files
...
if 0:
    ...
    if 1: #plot of Ferguson method
        plt.scatter(logEps0, logSumA0, s=1, c='C0', edgecolor='C0', zorder=.1)
...
eps = 1e10 #in optimal range for PD_001; alter based on above plot
...
def is_symmetric(A):
    # for positive semidefinite, need to additionally check
    # ...all eigenvalues are (significantly) nonnegative
...
np.save(os.path.join(outDir, 'PD%s_tau%s_val.npy' % (PD, tau)), sdiag)
np.save(os.path.join(outDir, 'PD%s_tau%s_vec.npy' % (PD, tau)), U)
